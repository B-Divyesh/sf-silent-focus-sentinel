...
function home() {
  return shell(`
    ...
    ${trace()}
    <section class="workflow" aria-labelledby="workflow-title"><span class="eyebrow">THREE COMMANDS</span><h2 id="workflow-title">Move from swipe order to review</h2><ol><li><span>01</span><div><h3>Record the traversal</h3><p>Run your XCTest script and capture each ordered focus event.</p><code>silent-focus-sentinel record --command "…"</code></div></li><li><span>02</span><div><h3>Check each stop</h3><p>Find empty speech and repeated adjacent announcements.</p><code>silent-focus-sentinel analyze trace.json</code></div></li><li><span>03</span><div><h3>Compare the release</h3><p>Review new and resolved findings in CI.</p><code>silent-focus-sentinel diff baseline.json current.json</code></div></li></ol></section>
...
HTML: /tmp/…/focus-report.html</pre></div></section>
    <section class="limits" aria-labelledby="limits-title"><div><span class="eyebrow">CLEAR BOUNDARIES</span><h2 id="limits-title">Your script drives the simulator</h2></div><div><p>The tool reads ordered events from XCTest or another runner. It does not call private VoiceOver APIs.</p><p>It checks silent and repeated speech. It does not certify WCAG conformance.</p><p>Set <code>ignored: true</code> for an intentional decorative stop.</p></div></section>
